...
# Data: needs campaign (SIA) data:  https://montagu.vaccineimpact.org >> Scenario >> Download combined coverage set data
# 
...
# Note -  VIMC 2019 runs include 112 countries
...
#years <- c(2000:2100) #vector: c(first:last)

# set wokring directory depending on whether running locally or saving to OneDrive
run.local <- T
if (run.local){
  wd <- "/Users/petra/Documents/Projects/VIMC/VIMC-2019runs/"
  wd.noslash <- "/Users/petra/Documents/Projects/VIMC/VIMC-2019runs"
} else {
  wd <- "/Users/petra/OneDrive - London School of Hygiene and Tropical Medicine/VIMC/"
  wd.noslash <- "/Users/petra/OneDrive - London School of Hygiene and Tropical Medicine/VIMC"
}
...
output_dir <- paste0(wd, "generate_coverage/coverage/") 
...
sia2 <- sia2[,c("Geography","country_code","vaccine","Gavi73","Gavi-supported","Activity","vaccine","year","a0","a1","Extent","target","reached","coverage")]


#write data
sia2[, vaccine := NULL]
fwrite(sia2, paste0(output_dir,"coverage_sia_",scenario,".csv"),row.names=F)
fwrite(routine, paste0(output_dir,"coverage_routine_", scenario,".csv"))

#clean environment (remove variables, not data)
rm(list = ls()[which(!(ls() %in% c("sia","sia2","routine")))])
