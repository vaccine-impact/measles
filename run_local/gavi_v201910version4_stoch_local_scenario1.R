...
#       d) MCV1 and MCV2 dependency - give MCV2 to those who received MCV1 for MCV2 coverage <= MCV1, if MCV2 coverage > MCV1 distribute the remainder randomly (change in fortran code)
#       e) MCV1 and SIA dependency - use linear regression of data from Portnoy et al 2018 to inform the proportion of zero-dose children reached by SIA campaign (change in fortran code)
#       f) due to issues with age_range_verbatim field in the scenario (coverage) input files, these runs use age_first and age_last
#       g) uses time varying life-expectancy (for a given year) and actual age of death
...
if (run.local){
  wd <- paste0("/Users/petra/Documents/Projects/VIMC/VIMC-2019runs/", version, "/")
  wd.noslash <- paste0("/Users/petra/Documents/Projects/VIMC/VIMC-2019runs/", version)
} else {
...
# scenario index to run - this is the only bit you have to change
index <- 1
...
psa 		    <- 200		#Number of PSAs to run (0 to use deterministic).
...
if (index %in% c(1,5)) data_coverage_routine 	<- paste0("scenarios2019/coverage_routine_", scenario.name[2], ".csv")
...
data_gavi_template <- "stochastic-burden-template_2019104.csv" 
...
measles_model <- "vaccine2019_sia" # change this to reflect the right version of the fortran code
...
setwd(wd)			
source(paste0("./gavi_setup_v", version, ".R"))
#log
...
#foreach will run countries and PSA runs in parralel if a parallel backend is registered, and sequentially otherwise
combine <- foreach(
  ...
  out_run <- runCountry(
    ii, countries, years,
    vaccination, using_sia, dinf, gamma, r0_basic, amplitude, take, degree, sia_method,
    coverage_routine, coverage_sia, timeliness, contact, rnought, population, lexp, cfr,
    tstep,save.scenario, foldername,  measles_model, debug_model, debug_spinup, debug_age, 
    debug_compartments, debug_country, debug_relative, debug_timepoints,
    r, runs, psa, psa_var,
    process_results, run_model, remove_files
  )
  return(out_run)
}
...
all_cases  <- rbindlist(lapply(myfiles, function(fn, ...) {
  res <- withCallingHandlers(
    fread(fn, stringsAsFactors=F, check.names = F, fill = T, col.names = as.character(c(0:100))),
...
setorder(all_popsize, country)


all_cases.m <- melt(all_cases, id.vars = c("year","country", "run_id"), 
                    measure.vars = c(as.character(0:100)), 
                    variable.name = "age", value.name = "cases", variable.factor = F)

all_popsize.m <- melt(all_popsize, id.vars = c("year", "country", "run_id"),
                      measure.vars = c(as.character(0:100)), 
                      variable.name = "age", value.name = "cohort_size", variable.factor = F)
# melt produces character variable when variable.factor is set to FALSE (to avoid producing factors) - change it to integer
...
all_runs = merge(all_cases.m, all_popsize.m, by = c("year", "age", "country", "run_id"), all.x = T)
# add country_name and disease (Measles) to match template file
...
all_runs <- merge(all_runs, subset(life.exp, select= c("country_code", "year", "LE")), 
...
fwrite(output_runs[order(country, year, age)], 
       paste0(wd,"outcome/",save.scenario, "/",foldername, "/", "burden_template_Measles-LSHTM-Jit-", scenario, ".csv"))
